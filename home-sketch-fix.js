/* Cache-safe homepage artwork layer — 2026-08-17.
   The two decision illustrations are rendered directly on canvas so the live
   site cannot fall back to older clean/vector-looking assets on mobile. */
(function(){
  const INK = '#2c251f';
  const PAPER = '#fdf6ea';
  const PINK = '#ec91a6';
  ...
  function paperGrain(ctx,w,h,rand){
    ...
    for(let i=0;i<120;i++){
      ...
      ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(Math.min(w,x+len),y+(rand()-.5)*2); ctx.stroke();
    }
    ctx.restore();
  }
  ...
  function drawPriorities(host){
    ...
    const rand=rng(4117);
    ctx.fillStyle=PAPER; ctx.fillRect(0,0,1200,700); paperGrain(ctx,1200,700,rand);
  ...
  function drawDecision(host){
    ...
    const ctx=canvas.getContext('2d');
    const rand=rng(7821);
    ctx.fillStyle=PAPER; ctx.fillRect(0,0,1200,620); paperGrain(ctx,1200,620,rand);
  ...
  document.querySelectorAll('.gl-img').forEach((el)=>{ if(!el.classList.contains('art-glimpse-01')) el.style.setProperty('background-image','url("assets/dmt/glimpses-refined.avif?v=20260813d")','important'); });
  document.querySelectorAll('.pr-img').forEach((el)=>{ if(!el.classList.contains('art-problem-03')) el.style.setProperty('background-image','url("assets/dmt/problems-refined.avif?v=20260813d")','important'); });

  function render(){
    const priorities=document.querySelector('.art-glimpse-01'); const decision=document.querySelector('.art-problem-03');
    if(priorities) drawPriorities(priorities); if(decision) drawDecision(decision);
  }
